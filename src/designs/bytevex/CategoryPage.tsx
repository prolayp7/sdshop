...
import { ArrowRight, ChevronDown, Grid2X2, List, Package, SlidersHorizontal, X } from "lucide-react";
...
import "./category-redesign.css";
import gridStyles from "./category-grid.module.css";
...
export default function CategoryPage({ category, tree, initialMin, initialMax, initialSort, deals, initialProducts, benefits, initialQuery = "" }: {
  ...
  const [draftMin, setDraftMin] = useState(validPrice(initialMin));
  const [draftMax, setDraftMax] = useState(validPrice(initialMax));
  ...
  const reset = () => { setBrands([]); setSpecs({}); setPriceMin(""); setPriceMax(""); setDraftMin(""); setDraftMax(""); setStock(false); };
  const applyPrice = () => { setPriceMin(draftMin); setPriceMax(draftMax); };
  ...
  const activeFilters = [
    ...
    ...(priceMin !== "" || priceMax !== "" ? [{
      ...
      remove: () => { setPriceMin(""); setPriceMax(""); setDraftMin(""); setDraftMax(""); },
    }] : []),
  ...
  return <>
    <Header />
    <div className="category-page bytevex-category">
      ...
      <section className="category-heading"><div className="category-wrap category-heading-layout"><div><div className="category-eyebrow">PROFESSIONAL MEDIA <span>｜</span> LAB VERIFIED STORAGE</div>
        <div className="category-title"><h1>{label}</h1>{facetsRes.data ? <span>{facetsRes.data.meta.total.toLocaleString("en-GB")} Products available</span> : null}</div>
        {category?.description ? <p>{plainText(category.description)}</p> : <p>High-performance memory cards, storage and accessories for creators and professionals.</p>}
      ...
      <div className="category-catalogue" id="category-results"><div className="category-wrap category-layout">
        ...
        <aside id="category-filters" className={`category-sidebar${filtersOpen ? " is-open" : ""}`}>
          <div className="category-refine"><h2>Filter products</h2><span>{activeFilters.length ? `${activeFilters.length} selected` : "Choose filters"}</span></div>
          ...
          <details className="rs-group" open><summary>Price Range ({CURRENCY_SYMBOL})<ChevronDown size={14} /></summary>
            {maxPrice > 0 ? <div className="rs-dual">
              <div className="rs-track"><span style={{ left: `${(Number(draftMin || 0) / maxPrice) * 100}%`, right: `${100 - (Number(draftMax || maxPrice) / maxPrice) * 100}%` }} /></div>
              <input aria-label="Minimum price slider" type="range" min="0" max={maxPrice} value={draftMin || 0} onChange={(event) => setDraftMin(String(Math.min(Number(event.target.value), Number(draftMax || maxPrice))))} onPointerUp={applyPrice} onKeyUp={applyPrice} />
              <input aria-label="Maximum price slider" type="range" min="0" max={maxPrice} value={draftMax || maxPrice} onChange={(event) => setDraftMax(String(Math.max(Number(event.target.value), Number(draftMin || 0))))} onPointerUp={applyPrice} onKeyUp={applyPrice} />
            </div> : null}
            <div className="category-price-inputs"><label><b>{CURRENCY_SYMBOL}</b><input aria-label="Minimum price" type="number" min="0" placeholder="Min" value={draftMin} onChange={(event) => setDraftMin(event.target.value)} /></label><span>to</span><label><b>{CURRENCY_SYMBOL}</b><input aria-label="Maximum price" type="number" min={draftMin || "0"} placeholder="Max" value={draftMax} onChange={(event) => setDraftMax(event.target.value)} /></label><button type="button" className="rs-go" onClick={applyPrice}>Go</button></div>
          </details>
          {(facets?.specifications ?? []).filter((spec) => !["brand", "category"].includes(spec.title.toLowerCase())).map((spec) => <details className="rs-group" open key={spec.title}><summary>{spec.title.replace(/([a-z])([A-Z])/g, "$1 $2")}<ChevronDown size={14} /></summary><div className="rs-list rs-scroll">{spec.values.map(({ value, count }) => <label key={value} className="rs-check"><input type="checkbox" checked={specs[spec.title]?.includes(value) ?? false} onChange={() => { const next = { ...specs }; const selected = next[spec.title] ?? []; const values = selected.includes(value) ? selected.filter((v) => v !== value) : [...selected, value]; if (values.length) next[spec.title] = values; else delete next[spec.title]; setSpecs(next); }} /><span>{value}</span><small>{count}</small></label>)}</div></details>)}
          <details className="rs-group" open><summary>Availability<ChevronDown size={14} /></summary><div className="rs-list"><label className="rs-check"><input type="checkbox" checked={stock} onChange={(event) => { setStock(event.target.checked); }} /><span>In Stock Only</span></label></div></details>
        </aside>
        <section className="category-results" aria-label="Products" aria-busy={results.loading}>
          ...
          <div className="category-toolbar">
            ...
            <div className="category-page-size"><span>Show:</span>{[12, 24, 48].map((size) => <button type="button" key={size} aria-label={`Show ${size} products`} aria-pressed={perPage === size} onClick={() => setPerPage(size)}>{size}</button>)}</div>
          ...
          {filtered ? <div className="category-active-filters" aria-label="Active filters">
            <span className="category-active-label">Active filters:</span>
            ...
            <button className="category-clear-filters" onClick={reset}>Clear All ({activeFilters.length})</button>
          </div> : null}
          </div>
          <div className={`${gridStyles.grid} ${view === "list" ? gridStyles.list : ""}`}>
            {shown.map((product, index) => <Fragment key={product.id}><CategoryProductCard product={product} layout={view} />{index === (view === "grid" ? 3 : 2) && index < shown.length - 1 ? <aside className={gridStyles.guide} aria-label="BYTEVEX storage guide"><div><span>BYTEVEX STORAGE GUIDE</span><h2>Choose the right media for your camera and workflow.</h2><p>Compare card format, sustained write speed and capacity before your next shoot.</p></div><Link href="/blog">Read the buying guide <ArrowRight size={17} /></Link></aside> : null}</Fragment>)}
            {results.loading ? Array.from({ length: perPage }, (_, index) => <div key={`skeleton-${index}`} className={gridStyles.skeleton} aria-hidden="true"><div /><span /><span /><span /></div>) : null}
          </div>
