...
import { ArrowRight, ChevronDown, Grid2X2, List, Package, Search, SlidersHorizontal, X } from "lucide-react";
...
export default function CategoryPage({ category, tree, initialMin, initialMax, initialSort, deals, initialProducts, benefits, initialQuery = "" }: {
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => setWithinQuery(within.trim()), 350);
  ...
  if (withinQuery) query.set("q", withinQuery);
  ...
  const reset = () => { setBrands([]); setSpecs({}); setPriceMin(""); setPriceMax(""); setDraftMin(""); setDraftMax(""); setStock(false); setWithin(""); setWithinQuery(""); };
      ...
      <div className="category-catalogue" id="category-results"><div className="category-wrap category-layout">
        ...
        <section className="category-results" aria-label="Products" aria-busy={results.loading}>
          ...
          <div className="category-toolbar">
            ...
            <label className="category-within"><Search size={14} /><input type="search" placeholder={`Search within ${category?.title || "products"}...`} aria-label="Search within results" value={within} onChange={(event) => setWithin(event.target.value)} /></label>
          ...
          {filtered || withinQuery ? <div className="category-active-filters" aria-label="Active filters">
