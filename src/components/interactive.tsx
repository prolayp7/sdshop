...
interface AddToBasketProps extends BaseButtonProps {
  ...
  /** Render with the magicui interactive-hover-button effect (product grids). */
  hover?: boolean;
}

const HOVER_ADD = "min-h-10! w-full! rounded-[10px]! border! border-[#087ff5]! bg-white! px-3! py-2! text-[12px]! font-bold! text-[#087ff5]! disabled:pointer-events-none disabled:opacity-50";
...
export function AddToBasketButton({ product, qty, qtyInputRef, hover, className, children, disabled, ...rest }: AddToBasketProps) {
  ...
  return hover
    ? <InteractiveHoverButton type="button" {...props} className={cn(HOVER_ADD, className)}><span className="inline-flex items-center gap-2">{children}</span></InteractiveHoverButton>
